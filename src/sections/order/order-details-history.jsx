...
export function OrderDetailsHistory({ history }) {
  const renderSummary = (
    ...
    >
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>زمان سفارش</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>تاریخ پرداخت</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>زمان تحویل برای حامل</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>زمان تکمیل</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
    </Paper>
  );
