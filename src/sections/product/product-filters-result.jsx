...
export function ProductFiltersResult({ filters, totalResults, sx }) {
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={filters.onResetState} sx={sx}>
      <FiltersBlock label="جنسیت:" isShow={!!filters.state.gender.length}>
        {filters.state.gender.map((item) => (
          <Chip {...chipProps} key={item} label={item} onDelete={() => handleRemoveGender(item)} />
        ))}
      </FiltersBlock>

      <FiltersBlock label="دسته بندی:" isShow={filters.state.category !== 'all'}>
        <Chip {...chipProps} label={filters.state.category} onDelete={handleRemoveCategory} />
      </FiltersBlock>

      <FiltersBlock label="رنگ:" isShow={!!filters.state.colors.length}>
      ...
      <FiltersBlock label="امتیاز:" isShow={!!filters.state.rating}>
