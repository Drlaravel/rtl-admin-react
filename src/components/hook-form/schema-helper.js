...
export const schemaHelper = {
  ...
  phoneNumber: (props) =>
    zod
      .string()
      .min(1, {
        message: props?.message?.required_error ?? 'شماره موبایل الزامیه!',
      })
      .refine((data) => props?.isValidPhoneNumber?.(data), {
        message: props?.message?.invalid_type_error ?? 'شماره موبایل معتبر نیست!',
      }),
  /**
   * date
   * defaultValue === null
      ...
      .transform((dateString, ctx) => {
        ...
        if (!dateString) {
          ctx.addIssue({
            ...
            message: props?.message?.required_error ?? 'تاریخ الزامیه!',
          });
          return null;
        }
        ...
        if (!stringToDate.safeParse(date).success) {
          ctx.addIssue({
            ...
            message: props?.message?.invalid_type_error ?? 'تاریخ معتبر نیست!!',
          });
        }
  ...
  editor: (props) =>
    zod.string().min(8, {
      message: props?.message?.required_error ?? 'ادیتور !الزامی است',
    }),
  /**
   * object
   * defaultValue === null
  ...
  boolean: (props) =>
    zod.coerce.boolean().refine((bool) => bool === true, {
      message: props?.message?.required_error ?? 'سویچ !الزامی است',
    }),
  /**
   * file
   * defaultValue === '' || null
  ...
  files: (props) =>
    zod.array(zod.custom()).transform((data, ctx) => {
      ...
      if (!data.length) {
        ...
        ctx.addIssue({
          code: zod.ZodIssueCode.custom,
          message: `باید حداقل ${minFiles} مورد داشته باشد!`,
        });
      }
