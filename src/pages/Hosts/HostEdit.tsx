    ...
    useEffect(() => {
        const fetchHostData = async () => {
            try {
                ...
                setExpiryDate(hostData.expiry_date);
                setReminderDate(hostData.reminder_date);
                if (hostData.project_id) {
                    setValue('user_id', null);
                }
                // If the host is associated with a user, clear the project field
                else if (hostData.user_id) {
            ...
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <div className="p-6.5">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="my-4.5">
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    setValue('associated_with', value); // به‌روزرسانی مقدار انتخاب‌شده
                                    if (value === 'project') {
                                        setValue('user_id', null); // اگر پروژه انتخاب شد، user_id پاک می‌شود
                                    } else if (value === 'user') {
                                        setValue('project_id', null); // اگر کاربر انتخاب شد، project_id پاک می‌شود
                                    }
                                }}
                        ...
                        {/* نمایش لیست پروژه‌ها یا کاربران بسته به انتخاب */}
                        {watch('associated_with') === 'project' && (
                            <div className="my-4.5">
                                <label className="block mb-2.5 text-black dark:text-white">انتخاب پروژه</label>
                                <Select
                                    styles={customStyles}
                                    options={projects}
                                    onChange={(option) => setValue('project_id', option?.value || null)} // مدیریت انتخاب پروژه
                        ...
                        {watch('associated_with') === 'user' && (
                            <div className="my-4.5">
                                <label className="block mb-2.5 text-black dark:text-white">انتخاب کاربر</label>
                                <Select
                                    styles={customStyles}
                                    options={users}
                                    onChange={(option) => setValue('user_id', option?.value || null)} // مدیریت انتخاب کاربر
